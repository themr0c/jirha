...
`jirha update KEY --sp auto` and `jirha hygiene` assess story points by analyzing all linked GitHub PRs. Hygiene auto-links unmatched PRs before SP reassessment (see [jirha-reference.md](jirha-reference.md) for matching rules). Hygiene only flags mismatches of 2+ tiers.
