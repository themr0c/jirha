...
def _build_fields(args, jira):
    ...
    if args.fix_version:
        existing = [{'name': v.name} for v in jira.issue(args.key, fields='fixVersions').fields.fixVersions]
    ...
    if getattr(args, 'affects_version', None):
        existing = [{'name': v.name} for v in jira.issue(args.key, fields='versions').fields.versions]
