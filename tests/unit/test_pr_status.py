...
class TestFetchPrChecklistComments:
    """Test that unresolved_comments uses GraphQL reviewThreads."""

    def _mock_gh_cli(self, data, returncode=0):
        result = MagicMock()
        result.returncode = returncode
        result.stdout = json.dumps(data)
        return result
    ...
    @patch("jirha.api.subprocess.run")
    def test_counts_unresolved_threads(self, mock_run):
        cli_data = {
            "state": "OPEN",
            "reviewDecision": "CHANGES_REQUESTED",
            "statusCheckRollup": [],
        ...
        mock_run.side_effect = [
            self._mock_gh_cli(cli_data),
            self._mock_gh_cli(graphql_data),
        ]
        from jirha.api import _fetch_pr_checklist

        checklist = _fetch_pr_checklist("https://github.com/org/repo/pull/42")
        assert checklist["unresolved_comments"] == 1

    @patch("jirha.api.subprocess.run")
    def test_all_resolved_is_zero(self, mock_run):
        cli_data = {
            "state": "OPEN",
            "reviewDecision": "APPROVED",
            "statusCheckRollup": [],
        ...
        checklist = _fetch_pr_checklist("https://github.com/org/repo/pull/10")
        assert checklist["unresolved_comments"] == 0

    @patch("jirha.api.subprocess.run")
    def test_graphql_failure_falls_back_to_zero(self, mock_run):
        cli_data = {
            "state": "OPEN",
            "reviewDecision": "",
            "statusCheckRollup": [],
        ...
        mock_run.side_effect = [
            self._mock_gh_cli(cli_data),
            self._mock_gh_cli({}, returncode=1),
        ]
        from jirha.api import _fetch_pr_checklist

        checklist = _fetch_pr_checklist("https://github.com/org/repo/pull/5")
        assert checklist["unresolved_comments"] == 0
